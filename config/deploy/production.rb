...
role :web,    "ec2-107-20-16-75.compute-1.amazonaws.com"
role :resque_worker, "ec2-107-20-16-75.compute-1.amazonaws.com"
role :app,    "ec2-107-20-16-75.compute-1.amazonaws.com"
role :db,     "ec2-107-20-16-75.compute-1.amazonaws.com", :primary => true
...
namespace :deploy do
  ...
  task :restart, :except => { :no_release => true, :no_unicorn => true } do
    run "kill -s USR2 `cat #{shared_path}/tmp/pids/unicorn.pid`"
  end

  desc "Start unicorn"
  task :start, :except => { :no_release => true, :no_unicorn => true } do
    run "cd #{current_path} ; bundle exec unicorn_rails -c config/unicorn.rb -E production -D"
  end

  desc "Stop unicorn"
  task :stop, :except => { :no_release => true, :no_unicorn => true } do
